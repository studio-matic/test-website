        ...
        Ok(_) => {
            ...
            (
                ...
                AppendHeaders([(
                    ...
                    format!("session_token={token}; Max-Age=86400; HttpOnly"),
                    #[cfg(not(debug_assertions))]
                    format!(
                        "session_token={token}; Max-Age=86400; HttpOnly; Secure; SameSite=None"
                    ),
                )]),
...
pub async fn cleanup_expired_sessions(pool: MySqlPool) {
    let mut interval = tokio::time::interval(Duration::from_hours(1)); // once per hour
    loop {
        interval.tick().await;

        match sqlx::query("DELETE FROM sessions WHERE created_at < NOW() - INTERVAL 1 HOUR")
            .execute(&pool)
            .await
        {
